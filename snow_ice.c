...
void generateLevel(int loops){
	...
	for (int j = 0; j < sizey; j++){
		for (int i = 0; i < sizex; i++){
			int r = rand()%100;
            int s = 13;
	        if (i < s || i >= sizex - s || j < s || j >= sizey - s) r = rand() % 100;
            else r = 0;
            s/=3;
            if (i < s || i >= sizex - s || j < s || j >= sizey - s) r = rand() % 100+20;
	        if (r <= 45) r = 0;
	        else r = 1;
    ...
    for (int y = 0; y < sizey; y++) {
    	for (int x = 0; x < sizex; x++){
	        ...
	        if (genTiles[n] == 0){
                int s = rand() % 30;
                W->tiles[y][x] = new Tile("Snow",  0,0,0,
                                            L" ",  255-s, 255-s, 255-s, "Small");

	        }  
	        if (genTiles[n] == 1){
	            W->tiles[y][x] = new Tile("Floor", 200,200,200,
	                             			L" ",  200,200,200, "Small");
	        }
	        if (genTiles[n] == 2){
                int r = rand() % 10;
                ...
                if (r > 0){
                    ...
                    W->tiles[y][x] = new Tile("Window",     100,100,255,
                                            L"[",       0,0,255, "Small");
            }  
            if (genTiles[n] == 3){
                ...
                int t = 255-(rand()%5)*10;
                int a = 100;
                W->tiles[y][x] = new Tile("Ice",     t-a,t-a,t,
                                            L"#",    s-a,s-a,s, "Small");
            }
	    	n++;
	...
	while(1){
		...
		for (int j = 0; j < LINES; j++){
		    for (int i = 0; i < COLS / 2; i++){
                ...
                wchar_t* dou = L" ";
                if (T->type == "Wall" && T->ext_ascii != L"║" && T->ext_ascii != L"╗" && T->ext_ascii != L"╝")
                    dou = L"═";
                if (T->type == "Window")
                     dou = L"]";
                ...
                if (px == x && py == y){
                    ...
                    init_pair(255, playerBit, playerBack);
