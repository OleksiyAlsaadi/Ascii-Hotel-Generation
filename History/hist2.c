		...
		for (int j = 0; j < LINES; j++){
		    for (int i = 0; i < COLS; i++){
                ...
                if (x >= 0 && x < sizex && y >= 0 && y < sizey && W->tiles[y][x] != NULL){
                    ...
                    autoTile = true;
                    // int a = sin( ridgenoise((x+1950)*.1, (y+1500)*.1, .2, 1) ) * 28;
                    int b = int(noise(y, x) * 2) % 2 + 1;
                    ...
                    if (y > -5 && y < 0){
                        ...
                        wchar_t* w = L" ";
                        if (b == 0) w = L"~";
                        T = new Tile("Water",  0,0,120,
                                        w,     0,0,80, "Small");
                    }

                    if (grass == 1){
                        int s = 255-d*10;
                        T = new Tile("Grass",  s,s,s,
                                        l,     s,s,s, "Small");
                    }  
                ...
                //Light calc
                float value;
                if (T->type == "Grass" || T->type == "Water")
                    value = 1;
                else{
                    value = calcLighting(x, y, 25);
                    if (value <= 0) value = 0;
                }
                value = 1;
